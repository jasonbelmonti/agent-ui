...
export function renderSignalButtonBuffer({ ctx, cols, rows, fillPercent, edgeWidthPx, sparkBurst, tone, timeMs, wakePercent, disabled = false, }) {
        ...
        for (let x = 0; x < cols; x += 1) {
            ...
            if (distance > -edgeCells) {
                ...
                if (distance > -1.25 && signalNoise > ghostThreshold) {
                    ...
                    drawPixel(ctx, x, y, accent, 0.72 + bodyBlend * 0.12, 0.04 + edgeProximity * 0.06, alpha);
                }
            }
        }
    }
...
function drawPulseRings(ctx, cols, rows, accent, burstLevel, disabled) {
    ...
    for (let y = 0; y < rows; y += 1) {
        for (let x = 0; x < cols; x += 1) {
            ...
            if (brightEnergy > 0.04) {
                drawPixel(ctx, x, y, accent, 1.02, 0.22 + brightEnergy * 0.32, clamp(baseAlpha + brightEnergy * 0.22, 0, 1));
            }
        }
    }
}
